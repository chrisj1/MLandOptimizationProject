...
fiberPropotion = [.001, .005, .01, .05, .1, .25, .5, 1]
...
def runTest(conf):
    ...
    print("Running trial with the following:\n\tProporion of Fibers = {}\n\tSize = {}\n\tRank = {}\n\tTrialNumber = {}\n\tMax Time = {}\n\tnu = {}\n\teps={}".format(fiberPropotion, Size, Rank, trial, maxtime,nu,eps))
    ...
    _,_,_,error,rates = CPDMWUTime(X, Rank, fiberPropotion, lamb, eps, nu, A_init, max_time,sample_interval=.5)
    return sum([error[x] for x in error.keys() if x > 5])/len([error[x] for x in error.keys() if x > 5])
cn_low = .2
cn_high= 100
