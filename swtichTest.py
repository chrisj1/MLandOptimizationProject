...
configurationada = collections.namedtuple('Configuration', 'X b0 fibers A B C eta Gt norm_x iterations start_time errors')
configurationcpdmwu = collections.namedtuple('Configuration', 'X Rank sketching_rates lamb eps eta A B C norm_x X_unfold iterations start_time errors')
...
for adaIters, cpdmwuIters in iters:
    ...
    conf1 = configurationada(X, b0, n_mb, A,B,C, eta_ada, Gt, norm_x, adaIters, start_time, errors)
    print(conf1)
    c, estimate, errors = adaIteration(conf1)
    A,B,C = estimate[0], estimate[1], estimate[2]
    conf2 = configurationcpdmwu(X, Rank, proprtions, lamb, eps, eta_cpd, A, B, C, norm_x, X_unfold, cpdmwuIters, start_time, errors)
