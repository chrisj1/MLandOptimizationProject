...
def adaIteration(configuration):
    X, b0, n_mb, A,B,C, eta, Gt, norm_x, iterations, start_time, errors = configuration
    estimate = [A,B,C]
    ...
    for it in range(iterations):
        ...
        g = (1 / n_mb) * (
            estimate[d_update] @ (H.transpose() @ H + mu * np.eye(F)) - X_sample.transpose() @ H - mu * estimate[d_update]
        )
...
def cpdmwuiteration(configuration):
    X, F, sketching_rates, lamb, eps, eta, A,B,C, norm_x, X_unfold, iterations, start_time, errors = configuration
    ...
    for i in range(iterations):
        ...
        if p == 1 and len(sketching_rates) > 1:
            update_weights(
                A, B, C, X_unfold, I, norm_x, lamb, weights, sketching_rates, F, eta, eps
            )
        now = time.time()            
