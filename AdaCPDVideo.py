...
def run(F):
	...
	time_A, NRE_A, A = AdaCPD(X, .1, 50, 30, Hinit, None)

	avg = sum(time_A)/len(time_A)

	return {'Decomposition Rank':F, 'Cost':NRE_A, 'Average MTTRK Time':avg, 'Time':time_A}
...
for F in Fs:
    ...
    ax.scatter([x for x in range(5, len(average))], average[5:], label="rank {}".format(F))
...
with open('AdaVideoData.csv', 'a') as file:
	...
	for data in datas:
		...
		points = Err_A
		ax.scatter([x for x in range(5, len(points))], points[5:], label="Rank {}".format(r))
